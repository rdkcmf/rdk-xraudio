...
##########################################################################
*/
#ifndef _XRAUDIO_LOG_H_
#define _XRAUDIO_LOG_H_


#define LOG_PREFIX       "XRAUDIO: "
#define LOG_PREFIX_FATAL "XRAUDIO:FATAL: "
#define LOG_PREFIX_ERROR "XRAUDIO:ERROR: "
#define LOG_PREFIX_WARN  "XRAUDIO:WARN: "
   ...
   #define LOG(...) if(b_rdk_logger_enabled) {\
                       RDK_LOG(RDK_LOG_DEBUG, "LOG.RDK.XRAUDIO", __VA_ARGS__);\
                    } else {\
                       char log_buffer[13]; xraudio_get_log_time(log_buffer); printf("%s " FORMAT, log_buffer, ##__VA_ARGS__); \
                    }
   #define LOG_FLUSH() fflush(stdout)
#else
   #define LOG(FORMAT, ...); {char log_buffer[13]; xraudio_get_log_time(log_buffer); printf("%s " FORMAT "\n", log_buffer, ##__VA_ARGS__);}
   #define LOG_FLUSH() fflush(stdout)
#endif
